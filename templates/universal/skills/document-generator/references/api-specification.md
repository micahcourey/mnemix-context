...
**Errors**: `404 NOT_FOUND` if resource does not exist or belongs to a different tenant.
...
| `tenantId` | integer | Yes | Must match user's tenant |
...
All endpoints automatically filter by `tenant_id` based on the authenticated user's tenant assignment. Users cannot access resources belonging to other tenants.
