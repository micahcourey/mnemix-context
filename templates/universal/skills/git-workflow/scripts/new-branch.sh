...
#
# Usage: ./new-branch.sh {{conventions.branch_pattern}}
#
# This script:
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "Examples:"
    echo "  $0 {{project.jira_key}}-1234"
    echo "  $0 {{project.jira_key}}-5678"
    exit 1
fi
...
if [[ ! "$BRANCH_NAME" =~ ^{{project.jira_key}}-[0-9]+$ ]]; then
    warn "Branch name '$BRANCH_NAME' doesn't follow {{conventions.branch_pattern}} format"
